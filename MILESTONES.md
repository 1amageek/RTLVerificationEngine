...
| M3 | Constraint-aware CDC/RDC | M1 frontend and declared clocks/resets | SDC projection, clock/reset graph, synchronizer and reset-release evidence | CDC clock coverage and order-independent source-domain crossings implemented; RDC clock-domain blockers added; reset intent and full exception semantics remain in progress |
...
| M6 | Process/tool qualification | M5 evidence | ToolQualification descriptor, health check, scope, freshness and PDK/deck evidence | Scope, freshness and corpus/oracle/health evidence binding contracts complete; process evidence pending |
| M7 | Headless flow and human review | M6 selection policy | Xcircuite stage adapter, immutable artifacts, resume/cancel, review bundle and approval gate | Serial Xcircuite regression passes 534 tests in 58 suites, including RTL stage/resume, LogicEngine bridge, review and end-to-end contracts; full workspace qualification remains open |
