...
1. Contract integrity and explicit evidence maturity (M0)
...
7. ToolQualification trust handoff and process-scoped evidence (M6)
...
- Multi-file implementation and reference source sets with deterministic defines, `ifdef`/`ifndef`/`elsif`/`else` conditional compilation, include resolution, include-cycle diagnostics and source provenance.
...
- Deterministic `rtl-verify` CLI with repeated RTL/reference inputs, SDC, frontend policy and `--record-input` artifact loading.
...
- Evidence maturity and observations are part of the public result contract.
  Native execution remains `unassessed` until retained evidence is attached;
  ToolQualification decisions are injected separately for external backends.

## External evidence and trust slice still required
...
- Record process-specific evidence for every supported PDK and solver configuration; the process scope/record enforces freshness, but no PDK evidence is supplied.
- Supply those evidence records to ToolQualification before an integrating flow accepts the external implementation.
...
- Extend the persisted corpus-run artifact pattern to oracle and process evidence,
  then let Xcircuite bind those immutable artifacts to its audit packet. Typed
  corpus results and the aggregate `RTLVerificationCorpusRun` are persisted by
  the native runner.
...
- No result claims ToolQualification acceptance or release eligibility.
