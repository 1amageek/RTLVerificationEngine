...
| Contract test | Passed | timeout-bounded SwiftPM test: 63 tests in 6 suites; qualification-input health identity binding, mapped pass/mismatch, typed counterexample differences, persisted corpus runs, oracle evidence artifacts, versioned lint rule catalog, canonical frontend parameterized hierarchy/case statements, hierarchy flattening, conditional elsif selection, retained process evidence binding, order-independent CDC domains, conservative RDC reset-release synchronizer recognition, mixed-domain blockers, top policy, freshness, scope binding, external descriptor identity, exact request-digest binding, independent oracle execution, solver proof artifact binding and timeout fixtures included |
...
| Process qualification | Retained evidence contract hardened, process evidence pending | Process scope/record requires a current `RTLVerificationProcessQualificationEvidence` artifact, a valid qualification window and matching retained corpus/oracle/health evidence IDs; health evidence must match implementation ID/version; no PDK-scoped qualification record |
...
- No PDK/process-specific qualification record has been supplied; a bare process record is insufficient, and stale, missing or implementation-mismatched retained process/health evidence is rejected.
- The external adapter is contract-complete but requires a qualified command descriptor.
- The Xcircuite serial full test graph passes 550 tests in 59 suites. A parallel run in the shared workspace produced transient cross-process failures, so it is not used as signoff evidence. The Xcircuite package is intentionally not committed with this repository's focused changes because its worktree contains unrelated in-progress changes.
